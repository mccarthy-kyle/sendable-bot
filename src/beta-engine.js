...
Respond with JSON ONLY, no markdown fence, this exact shape:
{
  ...
  "hazards": "ONLY genuinely DANGEROUS, consequential hazards for THIS route/date — the kind that injure or kill: steep snow you could slide down (runout into rocks/trees), need to self-arrest, exposed scrambling, river/creek crossings, cornices, rockfall, lightning on exposed terrain, whiteout/navigation. Most critical first, under ~30 words. NOT trivia like 'bring water' or 'trail is rocky'. If the route is genuinely clear with no consequential hazards, return an empty string.",
  "gear_kit": "Object with a kit ONLY IF real hazards exist — keyed by category, each a short comma list tied to the hazards. Categories (include only those that apply): traction (e.g. microspikes, crampons), safety (e.g. ice axe + know how to self-arrest, helmet, InReach), layers (e.g. wind shell, insulated layer for the summit), other (e.g. gaiters, trekking poles, extra water for a dry approach). If there are NO consequential hazards, return an empty object {}. Be specific: if there's steep snow, say 'ice axe AND the skill to self-arrest', not just 'ice axe'.",
